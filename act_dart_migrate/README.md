...
opt-in Dart SDK primary-constructor workaround for a stable-channel compiler
...
primary-constructor body would retain an explicit `super(...)` or
`super.named(...)` initializer. It does not skip classes merely because a
constructor uses super parameters such as `super.key`.
